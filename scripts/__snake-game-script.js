...
function getCommonDivisors(...nums) {
  ...
  return divisors[0].filter((num) =>
    divisors.slice(1).every((divisorList) => divisorList.includes(num))
  );
}
...
class Piece {
  ...
  scale(
    ...
    this.x =
      Math.round((this.x / prevDrawingSize / prevWidthUnits) * newWidthUnits) *
      newDrawingSize;
    this.y =
      Math.round(
        (this.y / prevDrawingSize / prevHeightUnits) * newHeightUnits
      ) * newDrawingSize;
  }
}
...
class Snake {
  ...
  renderFull(context, pieceSize, border) {
    this.segments.forEach((segment) =>
      segment.render(context, pieceSize, border)
    );
  }
  ...
  touchesGrid(width, height, drawingSize, border) {
    return (
      ...
      (this.segments[0].x == width - (drawingSize + border) &&
        this.segments[0].direction == Direction.RIGHT) ||
      (this.segments[0].y == height - (drawingSize + border) &&
  ...
  touchesTail() {
    return this.segments
      ...
      .some(
        (segment) =>
          segment.x == this.segments[0].x && segment.y == this.segments[0].y
      );
  }
  ...
  scale(
    ...
    const coordinates = this.segments.map((segment) => {
      return { x: segment.x, y: segment.y };
    });

    this.segments[0].scale(
      prevDrawingSize,
      newDrawingSize,
      prevWidthUnits,
      newWidthUnits,
    ...
    for (let i = 1; i < this.segments.length; ++i) {
      ...
      if (this.segments[i].x < coordinate.x) {
        this.segments[i].x = this.segments[i - 1].x - newDrawingSize;
      } else if (this.segments[i].x > coordinate.x) {
        this.segments[i].x = this.segments[i - 1].x + newDrawingSize;
      } else {
        this.segments[i].x = this.segments[i - 1].x;
      }

      if (this.segments[i].y < coordinate.y) {
        this.segments[i].y = this.segments[i - 1].y - newDrawingSize;
      } else if (this.segments[i].y > coordinate.y) {
        this.segments[i].y = this.segments[i - 1].y + newDrawingSize;
      } else {
...
class Game {
  ...
  resize() {
    switch (this.gamestate) {
      ...
      default: {
        ...
        const prevWidthUnits =
          (this.canvas.width - this.border) / prevDrawingSize;
        const prevHeightUnits =
          (this.canvas.height - this.border) / prevDrawingSize;
        ...
        const newWidthUnits =
          (this.canvas.width - this.border) / newDrawingSize;
        const newHeightUnits =
          (this.canvas.height - this.border) / newDrawingSize;
  ...
  getPieceSize() {
    const minCanvasSize =
      Math.min(this.canvas.width, this.canvas.height) - this.border;
    const maxCanvasSize =
      Math.max(this.canvas.width, this.canvas.height) - this.border;
    ...
    const possibleMinValues = minDivisors.filter((divisor) =>
      minDivisors.includes(minCanvasSize / divisor)
    );
    const possibleMaxValues = maxDivisors.filter((divisor) =>
      maxDivisors.includes(maxCanvasSize / divisor)
    );

    const possibleValue = possibleMaxValues.findLast(
      (divisor) =>
        possibleMinValues.includes(divisor) &&
        divisor >= 0.02 * minCanvasSize &&
        divisor <= 0.05 * minCanvasSize
    );

    if (possibleValue) {
      return possibleValue - this.border;
    }
  ...
  initializeSnake() {
    const segments = Array.from({ length: 5 }, (_, idx) => {
      ...
      segment.x =
        centerX - (centerX % this.drawingSize) + idx * this.drawingSize;
  ...
  createBlock() {
    ...
    [block.x, block.y] = generateValidRandomNums(
      0,
      this.canvas.width - this.drawingSize,
      0,
      this.canvas.height - this.drawingSize,
      this.drawingSize
    );
  ...
  isEnd() {
    const touchesGrid = this.snake.touchesGrid(
      this.canvas.width,
      this.canvas.height,
      this.drawingSize,
      this.border
...
document.addEventListener("keydown", (state) => {
  if (tutorialMessage.open || alertMessage.open || gameOver.open) {
    return;
  }
...
function activateEasyButtonBackground() {
  const backgroundColor = styles.getPropertyValue("--background-color").trim();
  easyButton.style.background = styles
    .getPropertyValue("--light-green-color")
    .trim();
  mediumButton.style.background = backgroundColor;
  hardButton.style.background = backgroundColor;
}
...
function activateMediumButtonBackground() {
  const backgroundColor = styles.getPropertyValue("--background-color").trim();
  easyButton.style.background = backgroundColor;
  mediumButton.style.background = styles
    .getPropertyValue("--light-yellow-color")
    .trim();
  hardButton.style.background = backgroundColor;
}
...
function activateHardButtonBackground() {
  ...
  easyButton.style.background = backgroundColor;
  mediumButton.style.background = backgroundColor;
  hardButton.style.background = styles
    .getPropertyValue("--light-red-color")
    .trim();
}
...
function gameLoop(timestamp) {
  ...
  console.log(
    getComputedStyle(document.querySelector("#page")).getPropertyValue(
      "display"
    )
  );
