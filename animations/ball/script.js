...
class Ball {
  ...
  move() {
    this.x +=
      this.xVelocity * timePerFrame +
      (this.xAcceleration * timePerFrame * timePerFrame) / 2;
    this.y +=
      this.yVelocity * timePerFrame +
      (this.yAcceleration * timePerFrame * timePerFrame) / 2;
    ...
    if (this.xVelocity) {
      this.xAcceleration = -this.xVelocity / 100000;
    }
    if (this.yVelocity) {
      this.yAcceleration = gravity - this.yVelocity / 100000;
    }
  }
}
  ...
  return {
    ...
    draw(color = ballColor) {
      ...
      const hypotenuse = Math.sqrt(
        arrow.width * arrow.width + arrow.height * arrow.height
      );
      ...
      context.lineTo(
        hypotenuse - Math.min(10, 0.2 * hypotenuse),
        4 + Math.min(10, 0.2 * hypotenuse)
      );
      context.lineTo(
        hypotenuse - Math.min(10, 0.2 * hypotenuse),
        0 - Math.min(10, 0.2 * hypotenuse)
      );
      context.fill();
